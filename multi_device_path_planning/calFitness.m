function [fitness] = calFitness(path,Populations,indexforUAV,Info,seg_flag)
    global dsafe;    
    ...
    Distances=zeros(1,sizeofPop);
    for i=1:sizeofPop
        if(i~=indexforUAV)
            pathbest{i} = Populations{i}.BestPath;
            dx = diff(pathbest{i}(:,1));
            dy = diff(pathbest{i}(:,2));
            dz = diff(pathbest{i}(:,3));
            Distances(i)=sum(sqrt(dx.^2 + dy.^2 + dz.^2));
        end
    end
   ...
   % J4 - Smooth cost
    ...
    % 所有飞机求时间交集
    for UAV=1:sizeofPop
        if(UAV~=indexforUAV) && seg_flag{UAV}==0
    ...
    b4 = 2;
    % Overall cost
    fitness = b2*J2 + b4*J4+b3*(J2*J3)+J5;
end