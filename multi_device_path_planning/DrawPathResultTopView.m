...
for num=1:6
    ...
    if checkDistance(UAVpath{num,1}', UAVpath{num,2}', 1, 40) 
        plotAllFigure(Info,Info.X,Info.Y,Info.Z,UAVpath(num,:),x,y,z,u,v,w);
    end
%     set( h(num),'XTickLabel', [], 'YTickLabel', []);
%     xlabel('')
    lgd(num) = get(h(num), 'Legend');
    set(get(h(num), 'XLabel'), 'Position', [50 0 0]);
    set(get(h(num), 'YLabel'), 'Position', [-2 50 5]);
    axis tight;
    view(2);
...
set(h(1),'Position',[0.0347    0.5665    0.2843    0.3908]);
set(h(2),'Position',[0.3703    0.5533    0.2843    0.4022]);
set(h(3),'Position',[0.6950    0.5628    0.2809    0.3927]);
set(h(4),'Position',[0.0266    0.09    0.2877    0.3946]);
set(h(5),'Position',[0.3636    0.09    0.2877    0.3946]);
set(h(6),'Position',[0.6950    0.09    0.2877    0.3946]);        
% saveas(gcf,savepath);